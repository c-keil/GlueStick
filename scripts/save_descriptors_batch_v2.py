...
if __name__ == "__main__":
    ...
    freq = args.frequency
    disable_pbar = args.disable_progress
    save_batch = args.save_batch
    flatten_lines = args.flatten_lines
    ...
    image_files = image_files[::freq] #skip every other
    print("Running on {} images".format(len(image_files)))
    assert(len(image_files)>0)

    features_name = "gluestick_freq{}".format(freq)
    kp_dir = os.path.join(save_dir,features_name,'keypoints')
    desc_dir = os.path.join(save_dir,features_name,'descriptors')

    match_dir = os.path.join(save_dir,features_name,'matches')
    matched_desc_dir = os.path.join(save_dir,features_name,'matched_descriptors')
    matched_kp_dir = os.path.join(save_dir,features_name,'matched_keypoints')

    # matched_line_scores_dir = 
    line_dir = os.path.join(save_dir,features_name,'lines')
    matched_lines_dir = os.path.join(save_dir,features_name,'matched_lines')
    matched_line_indices_dir = os.path.join(save_dir,features_name,'matched_line_indices')
    matched_line_descs_dir = os.path.join(save_dir,features_name,'matched_line_descs')

    pred_dir = os.path.join(save_dir,features_name,'batch')
    mean_descs_dir = os.path.join(save_dir,features_name,'mean_descriptors')
    mean_line_descs_dir = os.path.join(save_dir,features_name,'mean_line_descriptors')
    match_scores_dir = os.path.join(save_dir,features_name,'match_scores')
    match_line_scores_dir = os.path.join(save_dir,features_name,'match_line_scores')
    ...
    os.makedirs(matched_desc_dir, exist_ok=True)
    os.makedirs(matched_kp_dir, exist_ok=True)
    ...
    img0_fname = image_files[0]
    img0_name = os.path.basename(img0_fname)[:-4]
    img0 = cv2.imread(img0_fname, 0)
    torch_gray0 = numpy_image_to_torch(img0).to('cuda')[None]

    for img1_fname in tqdm(image_files[1:], disable = disable_pbar):
        ...
        #save batch
        if save_batch:
            with open(os.path.join(pred_dir, img0_name + "_" + img1_name), 'wb') as p:
                pickle.dump(pred, p)
        ...
        np.save(os.path.join(matched_kp_dir, img0_name), matched_kps0)
        np.save(os.path.join(matched_desc_dir, img0_name), matched_desc0)
        np.save(os.path.join(matched_lines_dir, img0_name), matched_lines0)
        np.save(os.path.join(matched_line_descs_dir, img0_name), matched_line_descs0)
        np.save(os.path.join(matched_kp_dir, img1_name+"_"), matched_kps1)
        np.save(os.path.join(matched_desc_dir, img1_name+"_"), matched_desc1)
        ...
        #save mean descs
        np.save(os.path.join(mean_descs_dir, img0_name + "_" + img1_name), mean_descs)
        np.save(os.path.join(mean_line_descs_dir, img0_name + "_" + img1_name), mean_line_descs)
        #save scores
        np.save(os.path.join(match_scores_dir, img0_name), matched_scores_sorted)
        np.save(os.path.join(match_line_scores_dir, img0_name), matched_line_scores_sorted)
        

        #book keeping
        img0_fname = img1_fname
        img0_name = img1_name
        img0 = img1
        torch_gray0 = torch_gray1
        # quit()
    print("completed image set {} with freq {}".format(image_dir,freq))
